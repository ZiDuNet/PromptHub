...
| プレビュー       | [![Preview v0.5.8-beta.2](https://img.shields.io/badge/Preview-v0.5.8--beta.2-8B5CF6?style=for-the-badge&logo=github&logoColor=white)](https://github.com/legeling/PromptHub/releases/tag/v0.5.8-beta.2) | 現在のプレビュー版は `v0.5.8-beta.2` です。必要なら「設定 → このアプリについて」でプレビューチャンネルを有効化して自動チェックと更新も使えます。 |
...
### v0.5.8-beta.2 ← 最近のプレビュー版

- Skill ライフサイクル、プロジェクト / Agent 管理、プラットフォームのインストール / アンインストール経路をさらに強化
- Skill 管理の切り替えを横方向のトランジションに統一し、プロジェクト削除ボタンを destructive スタイルに変更
